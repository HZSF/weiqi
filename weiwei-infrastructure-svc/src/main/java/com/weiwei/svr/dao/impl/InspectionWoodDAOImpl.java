...
@Service
public class InspectionWoodDAOImpl extends JdbcDaoSupport implements IInspectionWoodDAO{
	
	@Autowired
	public InspectionWoodDAOImpl(DataSource dataSource){
	...
	public String insertNewWoodInspectionBooking(String username) {
		...
		if(result == null || result.size() == 0){
			String sql_insert = "INSERT INTO user_inspection_map (userName, isApplied_wood) VALUES (?, ?)";
			getJdbcTemplate().update(sql_insert, new Object[]{username, "Y"});
			return Constants.EVENT_SUCCESS;
		}else{
			String sql_query1 = "SELECT * FROM user_inspection_map WHERE userName=? AND isApplied_wood='Y'";
			...
			if(result1 == null || result1.size() == 0){
				String sql_update = "UPDATE user_inspection_map SET isApplied_wood = ? WHERE userName = ?";
